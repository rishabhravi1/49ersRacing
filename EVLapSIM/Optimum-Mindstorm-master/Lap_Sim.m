...
engineSpeed = [0:100:5500]; % RPM
% torque should be in N-m:
engineTq = [112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112 112];
primaryReduction = 1;
gear = [1/1]; % transmission gear ratios
finalDrive = 3.91; % large sprocket/small sprocket
shiftpoint = 5600; % optimal shiftpoint for most gears [RPM]
drivetrainLosses = .85; % percent of torque that makes it to the rear wheels 
shift_time = 0; % seconds
T_lock = 80; % differential locking torque (0 =  open, 1 = locked)
...
W = 750; % vehicle + driver weight (lbs)
WDF = 45; % front weight distribution (%)
cg = 12/12; % center of gravity height (ft)
...
if exist('EnduranceVelocityTrace.csv', 'file') == 2
    delete('EnduranceVelocityTrace.csv');
end

% Export data points to CSV file
data = [time_elapsed', velocity']; % concatenate distance and velocity columns
csvwrite('EnduranceVelocityTrace.csv', data)
...
% check if file already exists
if exist('AutocrossVelocityTrace.csv', 'file') == 2
    delete('AutocrossVelocityTrace.csv');
end

% Export data points to CSV file
data = [time_elapsed_ax', velocity_ax']; % concatenate distance and velocity columns
csvwrite('AutocrossVelocityTrace.csv', data)
...
% check if file already exists
if exist('EnduranceAccelerationTrace.csv', 'file') == 2
    delete('EnduranceAccelerationTrace.csv');
end


% Export data points to CSV file
data = [time_elapsed', acceleration']; % concatenate distance and velocity columns
csvwrite('EnduranceAccelerationTrace.csv', data)